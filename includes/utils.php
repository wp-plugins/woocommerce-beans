...
function print_beans($beans) {
    ...
    return "<span style='color: $color; font-style: italic;'> $beans $beans_name</span>";
}
...
function beans_info_tag($beans_address, $beans_rate){
    ...
    $link = get_permalink(get_option(WC_Beans_Settings::REWARD_PROGRAM_PAGE));
    $link = $link ? $link : "//".BEANS_WEBSITE."/$beans_address/";

    return "<div class='beans-info'>$info_html</div>".
           " <a target='_blank' href='$link'> ".
           __('More information.', 'woocommerce-beans').
           " </a> ";
}

function beans_join_button(){
    $button_text = __('Join', 'woocommerce-beans');
    $connect_mode = beans_get_opt('auto_enroll') ? 2:1;
    return "<button class='button' onclick='Beans.connect($connect_mode);' type='button'>$button_text</button>";
