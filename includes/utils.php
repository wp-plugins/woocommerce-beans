...
function beans_get_text($key){
    $company_name = beans_get_opt('company_name');
...
function beans_info_tag($beans_address, $beans_rate){
    if(beans_get_opt('beans_rate_msg')){
        $msg=beans_get_opt('beans_rate_msg1');

        if(!empty($msg)){
            $tags = array("[Beans]", "[CURRENCY]");
            $values  = array(print_beans($beans_rate), wc_price(1));
            $info_html= str_replace($tags, $values, $msg);
        }else{
            $info_html = sprintf(
