...
//                'beans_description_on_cart_page' => false,
//                'beans_rate_msg_text' => __('%1$s // %2$s', 'woocommerce-beans'),
            );
    ...
    public static function check_opt(){
        ...
        if(empty($opt['translation_done'])){
            ...
            $available_languages = array('en', ); // add here languages that have already been translated! Yeah!
    ...
    public function admin_notice() {
        ...
        if($this->opt['test_mode']){
            echo '<div class="error" style="margin-left: auto"><div style="margin: 10px auto;"> Beans: '.
                __( 'Beans is currently in Test Mode. Only the shop manager is able to see and use Beans.', 'woocommerce-beans-admin' ) .
                 '<form method="post" id="beans_activate_testing" style="display: inline;" action="' . admin_url( 'admin.php?page=wc-beans' ) . '" >'.
                     ...
                     ' <a href="#" onclick="document.getElementById(\'beans_activate_testing\').submit(); return false;">' .
                        __( 'Deactivate test mode', 'woocommerce-beans-admin' ) .
                     '</a>'.
                 '</form>'.
                '</div></div>';
        }
        ...
        {
            echo '<div class="error" style="margin-left: auto"><div style="margin: 10px auto;"> Beans: '.
                 __( 'You have not invited your customers to join your reward program.', 'woocommerce-beans-admin' ) .
        ...
        if(empty($this->opt['translation_done']))
        {
            ...
            echo '<div class="error" style="margin-left: auto"><div style="margin: 10px auto;"> Beans: '.
                ...
                '<form method="post" id="beans_translation_done" action="" >'.
                    ...
                    __( 'No thanks, I prefer English.', 'woocommerce-beans-admin' ) .
                    '</a>'.
                '</form>'.
                '</div></div>';
        }
        ...
        <div class="wrap">
            ...
            <?php if($this->opt['test_mode']) : ?>
                <div style="margin-top: 25px">
                    <h3><?php _e('Test Mode', 'woocommerce-beans-admin') ?></h3>
                    <div style="background-color: #FCEFA1; padding: 10px; border: 1px solid #DDDDDD; margin: 20px auto">
                        <?php
                            _e('Beans is currently in Test Mode. Only the shop manager is able to see and use Beans. '.
                                'The plugin is disabled for your customers and visitors. ', 'woocommerce-beans-admin');
                            // two parts translation to avoid repeat at the first text has already been translated
                            _e('When you are ready, just deactivate the test mode to allow your customers '.
                                'to see and join your reward program.', 'woocommerce-beans-admin' );
                        ?>
                    </div>

                    <form method="post" action="">
                        <input class='button' type='submit' name='test_mode'
                               value='<?php _e('Deactivate test mode', 'woocommerce-beans-admin' ); ?>'/>
                    </form>
                </div>
            ...
            <?php elseif(!$this->opt['invitation_sent']) : ?>
                ...
                <div style="margin-top: 25px">
                    <h3><?php _e('Invitations', 'woocommerce-beans-admin'); ?></h3>
            ...
            <?php endif; ?>
             ...
             <div style="margin-top: 25px">
                <h3><?php _e('Settings', 'woocommerce-beans-admin' ); ?></h3>
                <form method="post" action="">
                    ...
                    <?php if (!$this->opt['secret_key']) : ?>
                        <tr valign="top">
                            <td>
                                ...
                                <p class="description"><?php _e('Enter your Beans address.', 'woocommerce-beans-admin' ); ?></p>
                            </td>
                        </tr>
                        <tr valign="top">
                            <td>
                                ...
                                <p class="description"><?php _e('Enter your Beans secret key.', 'woocommerce-beans-admin' ); ?></p>
                            </td>
                        </tr>
                    <?php endif; ?>
<!--                        <tr valign="top">-->
...
<!--                                <label for="beans_description_on_cart_page_id" class="description">--><?php //_e('Show new beans description on cart page', 'woocommerce-beans-admin' ); ?><!--</label>-->
<!--                            </td>-->
<!--                        </tr>-->
                        <tr valign="top">
                            <td colspan="2">
                        ...
                        <tr valign="top">
                            <td colspan="2">
                                <input id="disable_product_page_id" type='checkbox' <?php if ($this->opt['disable_product_page']) { echo 'checked="checked"'; }; ?> name="disable_product_page"/>
                                <label for="disable_product_page_id" class="description"><?php _e('Disable Beans on product page', 'woocommerce-beans-admin' ); ?></label>
                            </td>
                        </tr>
                        <tr valign="top">
                            <td colspan="2">
...
<!--                        <tr valign="top">-->
<!--                            <th scope="row">--><?php //_e('Information tag', 'woocommerce-beans-admin' ); ?><!--</th>-->
<!--                            <td colspan="2">-->
             ...
             <div style="margin: 1em auto">
                 ...
                 <form method="post" action="" id="beans_activate_testing" style="display: inline;">
                     <a href="#" onclick="document.getElementById('beans_activate_testing').submit(); return false;">
                         <?php
                             if($this->opt['test_mode'])
                                 _e('Deactivate test mode', 'woocommerce-beans-admin' );
                             else
                                 _e('Activate test mode', 'woocommerce-beans-admin' );
                         ?>
                     </a>
                    <input type='hidden' name='test_mode' value='1'/>
                 </form>
                 ...
                 <a href='https://wordpress.org/support/view/plugin-reviews/woocommerce-beans#postform' target="_blank">
                     <?php _e('Make a review', 'woocommerce-beans-admin' ); ?>
                 </a>
                 <span style="margin: auto 5px">|</span>
                 <a href="https://wordpress.org/support/plugin/woocommerce-beans" target="_blank">
                     <?php _e('Help', 'woocommerce-beans-admin' ); ?>
                 </a>
                 <span style="margin: auto 5px">|</span>
    ...
    public static  function render_order_meta($post){
        ...
        <div id="beans-order-meta-wrapper">
            ...
            <?php if (!$beans_order->wc_order_id) : ?>
                <p><?php _e('No information could be found on this order.', 'woocommerce-beans-admin') ?></p>
            <?php elseif ($beans_order->account_id) : ?>
<!--                credit-->
                <?php if ($credit) : ?>
                    <p>
                        <strong>
                            <?php _e('Credit', 'woocommerce-beans-admin') ?>: <em class="beans-drcr-status-<?php echo $credit['status'] ?>">
                                <?php echo $credit['status'] ?></em>
                        </strong>
                        <?php
                            if ($credit['status']=='committed'):
                                printf(__('The customer has been credited %1$s for this order.', 'woocommerce-beans-admin'), print_beans($credit['beans']));
                            elseif($credit['status']=='failed') :
                                _e('Crediting the account failed with the message:', 'woocommerce-beans-admin');
                ...
                <?php else : ?>
                    <p>
                        <strong><?php _e('Credit', 'woocommerce-beans-admin') ?>: <em class="beans-drcr-status-on_hold">On Hold</em></strong>
                        <?php _e('The customer will be credited when the order status will be updated to <b>processing</b> or <b>completed</b>.', 'woocommerce-beans-admin') ?>
                    </p>
                <?php endif; ?>
<!--                debit-->
                <?php if ($debit) : ?>
                    <p>
                        <strong>
                            <?php _e('Debit', 'woocommerce-beans-admin') ?>: <em class="beans-drcr-status-<?php echo $debit['status'] ?>">
                                <?php echo $debit['status'] ?></em>
                        </strong>
                        <?php
                            if ($debit['status']=='committed'):
                                printf(__('The customer has been debited %1$s for this order.', 'woocommerce-beans-admin'), print_beans($debit['beans']));
                            elseif($debit['status']=='failed') :
                                _e('Debiting the account failed with the message:', 'woocommerce-beans-admin');
                ...
                <?php else : ?>
                    <p>
                        <strong><?php _e('Debit', 'woocommerce-beans-admin') ?>: <em class="beans-drcr-status-none">None</em></strong>
                        <?php _e('There is no redeem with this order.', 'woocommerce-beans-admin') ?>
                    </p>
                <?php endif; ?>

                <div id="beans-view-account-div">
                    <a href="//<?php echo BEANS_BUSINESS_WEBSITE.'/accounts/'.$beans_order->account_id; ?>" target="_blank">
                    <?php _e('View this customer account', 'woocommerce-beans-admin') ?></a>
                </div>
<!--            no beans account-->
            <?php else : ?>

                <p><?php _e('Hmmmm... This customer seems to not be using Beans.', 'woocommerce-beans-admin') ?></p>

                <?php if (!$opt['beans_invite_after_purchase']) : ?>

                    <p>
                        <?php _e('You have disable email notification, so there was nothing we could do to keep them. :(', 'woocommerce-beans-admin') ?>
                        <a href="<?php echo admin_url('admin.php?page=wc-beans'); ?>"><?php _e('Settings', 'woocommerce-beans-admin') ?></a>
                    </p>

                <?php elseif($beans_order->followup_sent) : ?>

                    <p><?php _e("Don't Panic! We have sent an email to correct this, so they still have a chance to get their beans for this order.", 'woocommerce-beans-admin'); ?></p>
    ...
    public function update_settings()
    {
         ...
         $this->opt['disable_product_page'] = isset($_POST['disable_product_page']);
...
//         $this->opt['beans_description_on_cart_page'] = isset($_POST['beans_description_on_cart_page']);

//         $this->opt['beans_rate_msg_text'] = $_POST['beans_rate_msg_text'];
...
// todo: Show beans balance for each user on users page menu, optimize call for performance
