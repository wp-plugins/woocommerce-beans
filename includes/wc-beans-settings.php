...
class WC_Beans_Settings
{
    ...
    public function install()
    {
        ...
        if(!isset($opt)){
            ...
            $default_option = array(
                ...
                'public_key' => '',
                'private_key' => '',
                'modal_msg' => __('Would you like to get our Beans card for free?', BEANS_I18N)
            );
            add_option(self::OPT_NAME, $default_option);
            
        }elseif(!isset($opt['rule_id']) && $opt['secret_key']){
    ...
    public function action_links( $links ) {
        $links_to_add = array();
        $links_to_add[] = '<a href="' . admin_url( 'admin.php?page=wc-beans' ) . '">' . __( 'Settings', BEANS_I18N ) . '</a>';
        // $links_to_add[] = '<a href="' . esc_url('http://docs.woothemes.com/documentation/plugins/woocommerce/') . '">' . __( 'Docs', BEANS_I18N ) . '</a>';
        // $links_to_add[] = '<a href="' . esc_url('http://support.woothemes.com/') . '">' . __( 'Premium Support', BEANS_I18N ) . '</a>';
    ...
    public function admin_menu()
    {
        if ( current_user_can( 'manage_woocommerce' ) )
            add_submenu_page( 'woocommerce', __( 'Beans Settings', BEANS_I18N ), 'Beans' , 'manage_woocommerce', BEANS_I18N, array( $this, 'render_settings_page' ) );  
    }
    ...
    public function render_settings_page() 
    {
        ...
        <div class="wrap">
            <div style="margin-bottom: 40px">
                <h3>Beans</h3> 
                <a href="<?php echo BEANS_BUSINESS_WEBSITE ?>/login/" class="button button-primary" target="_blank"><?php _e('Connect to Beans', BEANS_I18N ); ?></a> 
            </div>
            ...
            <?php if (!$opt['secret_key']) : ?>
                <div style="background-color: white; padding: 10px">
                    <p class="main"><strong><?php _e('Get started with Beans', BEANS_I18N); ?></strong></p>
                    <span>
                      ...
                      <?php _e(' is an advanced loyalty and reward program that helps you engage'.
                               ...
                               ' Your reward program will be promoted on our website and indexed for SEO.', BEANS_I18N); ?>    
                    </span>
                    <p>
                      <a href="<?php echo BEANS_BUSINESS_WEBSITE ?>/register/" target="_blank" class="button button-primary">
                        <?php _e('Join for free', BEANS_I18N); ?>
                      </a> 
                    </p>
                 </div>
            <?php else : ?>
                 <div style="margin-top: 40px">
                    <h3>Invitations</h3>
                    <p><?php _e('Send an email invitation to your customers to join your reward program.', BEANS_I18N ); ?></p>
 
                    <form method="post" action="">
                        <input type="hidden" name="action" value="invite_customers">
                        <input type="submit" id="invite_customers" class="button" value="<?php _e('Send invitations', BEANS_I18N ); ?>"/>
                    </form>
                 </div>
            <?php endif; ?> 
             ...
             <div style="margin-top: 40px">
                ...
                <form method="post" action="">
                    ...
                    <table class="form-table">
                        <tr valign="top">
                            <th scope="row"><?php _e('Public Key', BEANS_I18N ); ?></th>
                            <td>
                                <input class="regular-text code" type="text" name="beans_public_key" required="" value="<?php echo $opt['public_key']; ?>" />
                                <p class="description"><?php _e('Connect to your Beans account to get your public key.', BEANS_I18N ); ?></p>
                            </td>
                        </tr>
                        <tr valign="top">
                            <th scope="row"><?php _e('Secret Key', BEANS_I18N ); ?></th>
                            <td>
                                <input class="regular-text code" type="text" name="beans_secret_key" required="" value="<?php echo $opt['secret_key']; ?>" />
                                <p class="description"><?php _e('Connect to your Beans account to get your secret key.', BEANS_I18N ); ?></p>
                            </td>
                        </tr>
                    </table>
                    <?php submit_button(); ?>
             ...
             <p>
                 <a href="https://wordpress.org/support/plugin/woocommerce-beans" target="_blank"><?php _e('Support and feedback to Beans', BEANS_I18N ); ?></a> 
             </p>
        </div>
        <?php
    }
    ...
    public function send_invitations(){
        ...
        $this->message = __('Invitations sent successfully.', BEANS_I18N);
    }
    ...
    public function update_settings()
    {
        ...
        if (!$_POST['beans_public_key'] || !$_POST['beans_secret_key'])
            $this->errors[] = __('Beans Public and Secret keys are mandatory.', BEANS_I18N );
        ...
        if(empty($this->errors))
            $this->message = __('Settings saved successfully.', BEANS_I18N);
