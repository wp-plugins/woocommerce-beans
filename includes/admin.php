...
class WC_Beans_Settings
{
    ...
    function __construct() 
    {
        ...
        if(isset($this->opt['beans_address'])){
            Beans::init($this->opt['secret_key']);
            $this->card = Beans::get('card/'.$this->opt['beans_address']);
        }
...
//         $color = "<span style='color: #00aa00'>&#9632;</span>";
    ...
    public function admin_menu()
    {
        ...
        if(!get_post( get_option(self::REWARD_PROGRAM_PAGE ))){
            require_once(WP_PLUGIN_DIR . '/woocommerce/includes/admin/wc-admin-functions.php');
            wc_create_page('beans_page', self::REWARD_PROGRAM_PAGE, 'Reward Program', '[beans_page]', 0);
//            $page_data = array(
//                'post_status'       => 'publish',
//                'post_type'         => 'page',
//                'post_author'       => 1,`
//                'post_name'         => 'beans_page',
//                'post_title'        => 'Reward Program',
//                'post_content'      => '[beans_page]',
//                'post_parent'       => 0,
//                'comment_status'    => 'closed'
//            );
//            $page_id = wp_insert_post( $page_data );
//            update_option( self::REWARD_PROGRAM_PAGE, $page_id );
        }
    }
    ...
    public function form_post_handler() {
        ...
        if(isset($_POST['translation_done']))
            $this->opt['translation_done'] = true;
    ...
    public function render_settings_page()
    {
        ...
        <div class="wrap">
            ...
            <?php if(empty($this->card['is_active'])) : ?>
                <div style="margin-top: 25px">
                    ...
                    <a class="button" href="//business.trybeans.com/" target="_blank">
                        <?php _e('Deactivate test mode', 'woocommerce-beans' ); ?>
                    </a>
                </div>
            ...
            <div style="margin-top: 25px">
                <h3><?php _e('Style', 'woocommerce-beans') ?></h3>
                <a class="button" href="<?php echo admin_url('plugin-editor.php?file=woocommerce-beans/local/beans.css'); ?>">
                    <?php _e('Edit CSS', 'woocommerce-beans' ); ?>
                </a>
            </div>
                                ...
                                <table class="radio-table">
                                    ...
                                    <tr>
                                        ...
                                        <td><label for="auto_enroll_0" style="display: inline-block"><?php _e('Auto enroll users with shop account in my reward program.', 'woocommerce-beans' ); ?>
                                                <br><span class="small-text"><?php _e('Existing shop members will automatically be enrolled and will receive a notification email to help them get started. New customers will be prompted to create a store account.', 'woocommerce-beans' ); ?> </span></label></td>
                                    </tr>
                                    <tr>
...
<!--            no beans account-->
    ...
    public function create_beans_accounts(){
        if($this->opt['auto_enroll_done']) return;

        if($this->opt['auto_enroll']  && !empty($this->card['is_active'])){
            $query_args = array(
                'meta_query' => array(
                    'relation' => 'AND',
                    array(
            ...
            $this->opt['auto_enroll_done'] = true;
            update_option(BEANS_OPT_NAME, $this->opt);
        }
