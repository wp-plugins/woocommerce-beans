...
class WC_Beans_Order {
    ...
    public function save(){
        ...
        if(!$this->account_id){
            ...
            $this->account_id = get_user_meta($wc_order->customer_user, 'beans_account_id', true);
        }
    ...
    public function make_credit(){
        ...
        if($total>0){
            ...
            $data = array(
                'quantity'      => $total,
                'rule_type'     => 'beans:currency_spent',
        ...
        foreach ($coupon_codes as $code) {
            ...
            if ($code === BEANS_COUPON_UID) {
                ...
                try {
                    ...
                    error_log($e, 3, BEANS_ERROR_LOG);
                    error_log(print_r($data, true), 3, BEANS_ERROR_LOG);
                    if(!$err_msg) $err_msg = "Beans error: Unable to debit your beans account.".$e->getMessage();
                }
    ...
    public function send_followup_email(){
        ...
        $rule = Beans::get('rule/beans:currency_spent');
    ...
    public function cancel(){
        if($this->credit_id){
            try{
                Beans::post('credit/'.$this->credit_id.'/cancel', null, false);
            }catch(Exception $e) {
                beans_log_info( "**************** ERROR *****************" );
                error_log( $e, 3, BEANS_ERROR_LOG );
            }
        ...
        if($this->debit_id){
            try{
                Beans::post('debit/'.$this->debit_id.'/cancel', null, false);
            }catch(Exception $e) {
                beans_log_info( "**************** ERROR *****************" );
                error_log( $e, 3, BEANS_ERROR_LOG );
            }
