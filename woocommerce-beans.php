...
class WC_Beans{
    ...
    public function update_order($order_id, $order_status, $new_status){
        $beans_order = new WC_Beans_Order($order_id);
        // todo: create a function for validating order status
    ...
    public function render_cart_checkout_page($page){
        ...
        if($_SESSION['beans_account'] && $page == 'account'){
            ...
            $form_html = "<a class='button'  target='_blank' onclick='Beans.show(); return false;' ".
                         "href='".BEANS_WEBSITE."/\$".$this->opt['card_name']."/'>$button_text</a>";
        }
    ...
    public function render_order_page($wc_order){
        ...
        if (!$beans_order->wc_order_id){
            ...
            $info_html = sprintf(
                __('Connect your Beans account to earn %1$s with this purchase.', 'woocommerce-beans'),
                print_beans($beans_to_credit)
            );
            $form_html = beans_connect_button();
        }
        ?>
        <h2>Beans Details</h2>
        <div class="beans-div-cart-page">
