   ...
   public function confirm_beans_transaction($order_id, $order_status, $new_status){
    ...
    public function render_cart_page($page){
        ...
        if ($this->beans_account__id && $_SESSION['beans_coupon_data']) {
            $info_html = sprintf( 
                __('You have chosen to redeem <span class="beans-unit"> %s beans</span>.',BEANS_I18N),
                 $_SESSION['beans_to_redeem']
            );
            $button_text = __('Cancel', BEANS_I18N);
            ...
            $account = Beans::get('account/'.$this->beans_account__id);
            $info_html = sprintf( 
                __('You have <span class="beans-unit"> %s beans</span>.',BEANS_I18N),
                $account['beans']
            );
            $button_text = __('Redeem my beans', BEANS_I18N);
            ...
            $info_html = __('Connect with Beans to get rewarded when you make a purchase, 
                             like our Facebook page and more...', BEANS_I18N);
            $button_text = __('Connect with Beans', BEANS_I18N);
                    ...
                    <td>
                        <div style="padding: 10px; text-align: left">
                            <?php
                                 ...
                                 echo "<br/>";
                                 printf(
                                 __('Earn <span class="beans-unit"> %s beans</span> with this purchase.', BEANS_I18N),
                                 $beans_to_earn); 
                            ?>
    ...
    public function render_account_page($page){
        ...
        <div class="beans-div-account-page">
            ...
            <a href="//<?php echo Beans::$domain."/$".$this->opt['card_name'] ?>" class="button" target="_blank"> 
                <?php _e('Beans account', BEANS_I18N ); ?>
            </a> 
        </div>
        <?php
    }
    ...
    public function render_product_page($page){
        ...
        <div  class="beans-div-product-page">
            <?php  
                 printf(
                 __('Get this product for <span class="beans-unit"> %s beans</span>.', BEANS_I18N),
                 $beans_to_buy); 
            ?>
            <br/>
            <?php  
                 printf(
                 __('Buy this product and earn <span class="beans-unit"> %s beans</span>.', BEANS_I18N),
                 $beans_to_earn); 
            ?>
            <br/>            
            <span class="beans-unit"> <?php echo $_SESSION['beans_rate'] ?> beans</span> 
