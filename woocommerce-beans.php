...
class WC_Beans{
    ...
    function __construct(){
        ...
        //add_action('woocommerce_after_checkout_validation',                   array( $this, 'checkout_validation'), 10, 3);
    ...
    public function init_session_hook($user_login, $user){
        ...
        if($user_id){
            $_SESSION['beans_account'] = WC_Beans_Settings::get_beans_account($user_id);
    ...
    public function init_beans_account(){
        ...
        if($_SESSION['beans_account'] && !$_SESSION['beans_account_in_db'] && $user_id){
            WC_Beans_Settings::add_beans_account($user_id, $_SESSION['beans_account']);
            $_SESSION['beans_account_in_db'] = True;
        }
    }
    ...
    public function redeem_beans(){
        ...
        if(!$_SESSION['beans_account']) return;
        ...
        $min_coupon = (int) ($min_coupon * $this->opt['range_min_redeem'] / 10000) * 100;
        ...
        $max_coupon = (int) ($max_coupon * $this->opt['range_max_redeem']  / 10000) *100;
    ...
    /*public function checkout_validation($order_id){
    ...
    public function process_beans_transaction($order_id){
        ...
        self::log_info("order_id = $order_id; user_id = ".get_current_user_id()."; beans_account = ".$_SESSION['beans_account']);

        if( !$_SESSION['beans_account'] )        return;

        $order = new WC_Order($order_id);
        WC_Beans_Settings::add_dr_cr($order_id, $_SESSION['beans_account']);
        ...
        foreach($coupon_codes as $code){
            ...
            if( $code === self::UID){
                ...
                $data = array(
                    ...
                    'account__id'   => $_SESSION['beans_account'],
            ...
            if($total>0 && isset($_SESSION['beans_rule_currency_spent'])){
                ...
                try{
                    ...
                    if($credit['status'] == 'failed'){
                        ...
                        // fixme: Why is this commented? We should better handle error and prevent the admin if an error occurred
                    }else{
    ...
    public function render_cart_checkout_page($page){
        ...
        if($_SESSION['beans_account'] && $page == 'account'){
            ...
            $form_html = "<a class='button'  target='_blank' onclick='Beans.show(); return false;' ".
                         "href='//www.beans.cards/\$".$this->opt['card_name']."/'>$button_text</a>";
        }
    ...
    public function run_debug(){
        ...
        self::log_info("Start debugging localization: /$locale/", true);
        $locale_path = WP_LANG_DIR . "/woocommerce-beans/$locale.mo";
        self::log_info("Localization path 1: /$locale_path/ exists=".file_exists($locale_path));
        $locale_path = plugin_dir_path(__FILE__) . "/languages/woocommerce-beans-$locale.mo";
        self::log_info("Localization path 2: /$locale_path/ exists=".file_exists($locale_path));
