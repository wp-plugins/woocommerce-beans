...
class WC_Beans{
    ...
    public function redeem_beans(){
        ...
        global $woocommerce;
        $min_coupon = (int) $this->opt['range_min_redeem'] * $woocommerce->cart->subtotal * $_SESSION['beans_rate'] / 100 ;
        
        if ($account['beans'] < $min_coupon ){
            wc_add_notice( sprintf( __( 'Not enough '.$_SESSION['beans_name'].' to redeem. Minimal redeem is '.$min_coupon.' '.$_SESSION['beans_name'], 'woocommerce-beans' ) ) ,'error' );
            return;
        }
        
        $max_coupon = (int) $this->opt['range_max_redeem'] * $woocommerce->cart->subtotal * $_SESSION['beans_rate'] / 100 ;
        
        if ($account['beans'] > $max_coupon ){
            wc_add_notice( sprintf( __( 'Maximal redeem is '.$max_coupon.' '.$_SESSION['beans_name'], 'woocommerce-beans' ) ) ,'success' );
        }
    ...
    public function get_beans_coupon($coupon, $coupon_code){
        ...
        $cart_beans_limit_percentage = $this->opt['range_max_redeem'] / 100;
    ...
    public function render_shop_page($page){
        ...
        <span  class="price beans-price">
            ...
            <?php
                printf(
                    __('<span class="beans-unit"> %1$s %2$s</span>', 'woocommerce-beans'),
                    number_format_drop_zero_decimals($beans_to_buy,BEANS_DECIMALS),
                    $_SESSION['beans_name']
                );
            ?>
    ...
    public function render_cart_checkout_page($page){
        ...
        if ($this->beans_account__id && $_SESSION['beans_coupon_data'] && $_SESSION['beans_to_redeem'] > 0) {
            $info_html = sprintf( 
                __('You have chosen to redeem <span class="beans-unit"> %1$s %2$s</span>.','woocommerce-beans'),
                number_format_drop_zero_decimals($_SESSION['beans_to_redeem'],BEANS_DECIMALS),
                $_SESSION['beans_name']
            );
            ...
            $account = Beans::get('account/'.$this->beans_account__id);
            $info_html = sprintf( 
                __('You have  %1$s %2$s.','woocommerce-beans'),
                '<span class="beans-unit">' .number_format_drop_zero_decimals($account['beans'],BEANS_DECIMALS),
                $_SESSION['beans_name']
            );
            $info_html .= '</span>';
            $button_text = __('Redeem my '.$_SESSION['beans_name'], 'woocommerce-beans');
            ...
            $form_html = "<button class='beans-button beans-connect' onclick='Beans.connect(1);' type='button' style=\"background-image: url('$img')\">".$button_text."</button>";               
        }
        ?>
                    ...
                    <td>
                        <div style="padding: 10px; text-align: left;" >
                            <?php
                                 ...
                                 echo "<br/>";
                                 printf(
                                 __('Earn <span class="beans-unit"> %1$s %2$s</span> with this purchase.', 'woocommerce-beans'),
                                 number_format_drop_zero_decimals($beans_to_earn,BEANS_DECIMALS),
                                 $_SESSION['beans_name']
                                 );
                                 
                            ?>
                            
                            <a class="beans-info" href="//www.beans.cards/$<?php echo $this->opt['card_name']; ?>/"  target="_blank"  onclick="Beans.show(); return false;">
                                <?php
                            
                                    printf(
                            
                                        __('<span class="beans-unit"> %1$s %2$s </span> // %3$s', 'woocommerce-beans'),

                                        number_format_drop_zero_decimals($_SESSION['beans_rate'],BEANS_DECIMALS),
                                        $_SESSION['beans_name'],
                                        wc_price(1)
                                    );
                            
                                ?>
                                <span class="beans-info-tag" >i</span>
                            </a>            
                        </div>
    ...
    public function render_product_page($page){
        ...
        if (!empty($min_price) && !empty($max_price)){
            ...
            $get_product_msg = sprintf(__('Get this product for  %1$s - %2$s %3$s', 'woocommerce-beans'),'<span class="beans-unit">'.number_format_drop_zero_decimals($beans_to_buy_min,BEANS_DECIMALS), number_format_drop_zero_decimals($beans_to_buy_max,BEANS_DECIMALS),$_SESSION['beans_name']);
            $buy_product_msg = sprintf(__('Buy this product and earn %1$s - %2$s %3$s', 'woocommerce-beans'),'<span class="beans-unit">'.number_format_drop_zero_decimals($beans_to_earn_min,BEANS_DECIMALS),number_format_drop_zero_decimals($beans_to_earn_max,BEANS_DECIMALS),$_SESSION['beans_name']);
            
        }else{
            $beans_to_earn  = (int) ($regular_price * $_SESSION['beans_rule_currency_spent']['beans']);
            $beans_to_buy   = (int) ($regular_price * $_SESSION['beans_rate']);
            
            $get_product_msg = sprintf(__('Get this product for  %1$s %2$s', 'woocommerce-beans'),'<span class="beans-unit">'. number_format_drop_zero_decimals($beans_to_buy,BEANS_DECIMALS),$_SESSION['beans_name']);
            $buy_product_msg = sprintf(__('Buy this product and earn %1$s %2$s', 'woocommerce-beans'),'<span class="beans-unit">'.number_format_drop_zero_decimals($beans_to_earn,BEANS_DECIMALS),$_SESSION['beans_name']);
