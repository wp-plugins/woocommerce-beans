...
define('BEANS_BUSINESS_WEBSITE',        'https://business.trybeans.com');
...
if ( ! class_exists( 'WC_Beans' ) ) :
    ...
    class WC_Beans{
        ...
        public function enqueue_scripts(){
            ...
            wp_enqueue_script('beans-wc-script', plugins_url( 'assets/js/bootstrap.min.js' , __FILE__ ));
            wp_enqueue_script('beans-wc-script', plugins_url( 'assets/js/jquery.min.js' , __FILE__ ));
            wp_localize_script('beans-wc-script', 'beans_data', array(
                'public_key'  =>  $this->opt['public_key'],
                'beans_popup' =>  $this->opt['beans_popup'],
            ));

            wp_enqueue_style( 'bootstrap-wc-style', plugins_url( 'assets/css/bootstrap.min.css' , __FILE__ ));
        ...
        $button_style = beans_get_opt('default_button_style') ? 'button' : 'beans-button';
            ...
            $form_html = "<a class='$button_style'  target='_blank' onclick='Beans.show(); return false;' ".
                         "href='".BEANS_WEBSITE."/\$".$this->opt['card_uid']."/'>$button_text</a>";
        }
                    ...
                    <div class="beans-info-contain">
                        <?php
                            ...
                            if($this->opt['beans_description_on_cart_page'] && $page != 'account' && $_SESSION['beans_account']){
                                $this->render_beans_description(self::get_current_cart()->subtotal * $_SESSION['beans_rate']);
                            }else{
                            echo $info_html;
                            if ($page != 'account'){
                                echo "<br/>";
                            ...
                            echo beans_info_tag($this->opt['card_uid'], $_SESSION['beans_rate']);
                        ?>
                    </div>
                </div>
    ...
    <?php
        ...
        public function render_beans_description($beans_to_buy){
                ...
                if($_SESSION['beans_to_redeem'] && $_SESSION['beans_coupon_data']){
                    ...
                    echo '<div class="progress" style="width:100%;height: 30px;background-color:#ffffff;box-shadow: none;">
                            <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="4" aria-valuemin="0" aria-valuemax="10" style="width:15px">
                            </div>
                            <div>&nbsp;'.$info_html.'</div>
                          </div>';
                }else{
                    ...
                    //TODO : factorise progress bar code
                    if($beans_amount < $min_beans_redeem){
                        echo '<div class="progress" style="width:100%;height: 30px;background-color:#ffffff;box-shadow: none;">
                            <div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="4" aria-valuemin="0" aria-valuemax="10" style="width:15px">
                            </div>
                            <div>&nbsp;'.$info_html.' Not enough to redeem.</div>
                          </div>';
                    }elseif($beans_amount<$max_beans_redeem){
                        echo '<div class="progress" style="width:100%;height: 30px;background-color:#ffffff;box-shadow: none;">
                            <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="4" aria-valuemin="0" aria-valuemax="10" style="width:15px">
                            </div>
                            <div>&nbsp;'.$info_html.' Redeem and get '.$percent_complete.'% discount.</div>
                          </div>';
                        ...
                        echo '<div class="progress" style="width:100%;height: 30px;background-color:#ffffff;box-shadow: none;">
                            <div class="progress-bar progress-bar-success progress-bar-striped active" role="progressbar" aria-valuenow="4" aria-valuemin="0" aria-valuemax="10" style="width:15px">
                            </div>
                            <div>&nbsp;'.$info_html.' Redeem and get '.$percent_complete.'% discount!!</div>
                          </div>';
                    }
                ...
                echo '<div class="progress" style="width:90%;height: 30px;background-color:#ffffff;box-shadow: none;">
                            <div class="progress-bar progress-bar-info" role="progressbar" aria-valuenow="4" aria-valuemin="0" aria-valuemax="10" style="width:15px">
                            </div>
                            <div>&nbsp;'.$msg_b_to_earn.'</div>
                          </div>';
