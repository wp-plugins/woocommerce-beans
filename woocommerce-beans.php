...
class WC_Beans{
    ...
    public function redeem_beans(){
        ...
        if ($account['beans'] < $min_coupon ){
            wc_add_notice( sprintf( __( 'Not enough Beans to redeem. Minimal redeem is '.$min_coupon.' Beans', 'woocommerce-beans' ) ) ,'error' );
            return;
        }
        ...
        if ($account['beans'] > $max_coupon ){
            wc_add_notice( sprintf( __( 'Maximal redeem is '.$max_coupon.' Beans', 'woocommerce-beans' ) ) ,'success' );
        }
...
$GLOBALS['wc_beans'] = wc_beans_instance();
 ...
 // TODO: Send an invitation to a customer (that doesn't have a beans account) just after completing an order
?>