...
//die(print_r(plugin_dir_path(__FILE__),true));
...
class WC_Beans{
    ...
    function __construct(){
        ...
        add_action('woocommerce_before_checkout_form',                        array( $this, 'render_checkout_page' ), 15);
    ...
    public function enqueue_scripts(){
        ...
        wp_localize_script('beans-wc-script', 'beans_data', array(
            ...
            'modal_msg'   =>  $this->opt['modal_msg'],
            'card_name'   =>  $this->opt['card_name'],
        ));
        wp_enqueue_style( 'beans-wc-style', plugins_url( 'assets/css/local.beans.css' , __FILE__ ));
        wp_enqueue_style( 'beans-wc-style', plugins_url( 'assets/css/master.beans.css' , __FILE__ ));
    }
   ...
   public function confirm_beans_transaction($order_id, $order_status, $new_status){
    ...
    public function render_checkout_page($page){
        global $woocommerce;
    ...
    public function render_product_page($page){
        ...
        if (!empty($min_price) && !empty($max_price)){
            ...
            $beans_to_earn_min  = (int) ($min_price * $_SESSION['beans_rule']['beans']);
            $beans_to_earn_max  = (int) ($max_price * $_SESSION['beans_rule']['beans']); 
            
            $beans_to_buy_min   = (int) ($min_price * $_SESSION['beans_rate']);
            $beans_to_buy_max   = (int) ($max_price * $_SESSION['beans_rate']);   
            
            $get_product_msg = sprintf(__('Get this product for  %1$s - %2$s beans', 'woocommerce-beans'),'<span class="beans-unit">'.$beans_to_earn_min,$beans_to_earn_max ); 
            $buy_product_msg = sprintf(__('Buy this product and earn %1$s - %2$s beans', 'woocommerce-beans'),'<span class="beans-unit">'.$beans_to_buy_min,$beans_to_buy_max); 
