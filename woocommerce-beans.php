...
if (!class_exists('BeansException',false))
    include_once(plugin_dir_path(__FILE__).'/includes/beans-api.php');

function plugin_version($plugin) {
    if ( ! function_exists( 'get_plugins' ) )
        require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
    $plugin_folder = get_plugins( '/'.$plugin );
    return $plugin_folder[$plugin.'.php']['Version'];
}

BeansAPI::$signature = '[CMS]Wordpress '.get_bloginfo('version').
                       ' WooCommerce '.plugin_version('woocommerce').
                       ' Beans Loyalty Program '.plugin_version('woocommerce-beans');

BeansAPI::set_error_log(3, BEANS_ERROR_LOG);
...
class WC_Beans{
    ...
    function __construct(){
        ...
        BeansAPI::init($this->opt['secret_key']);
        $this->card_check = BeansAPI::call('card/check/',null,false);
            
        if($this->card_check !== null)
            $this->is_ready = true;
    }
    ...
    public function update_cart(){
        ...
        if(!$this->reward)
            $this->reward = BeansAPI::call('reward/get/', array('accept_new_reward_type' => 1));
        
        //Check if reward can be used
        if ($this->reward && !BeansAPI::call('reward/check/')) 
            $this->reward = null;
        ...
        foreach($coupons_code as $code){
            ...
            if( strpos($code, self::uid) !== false)
                ...
                if(strpos($code, $reward_code_pattern) !== false)
                    ...
                    method_exists($woocommerce->cart, 'remove_coupon')?
    ...
    public function get_beans_coupon($coupon, $coupon_code){
        ...
        $reward_id = $tmp[1];
        $arg = array('reward' => $reward_id, 'accept_new_reward_type' => 1);
        
        if( (!$this->reward || $this->reward['id'] != $reward_id) )
            if(BeansAPI::call('reward/check/', $arg))
                $this->reward = BeansAPI::call('reward/get/', $arg);
            else
                return $coupon;
        ...
        switch ($reward['type']){
            ...
            case 'beans:cart_coupon' :
                $coupon_data['type'] = 'fixed_cart';
                if (get_woocommerce_currency() == $reward['attribute']['currency'])
                    $coupon_data['amount'] = $reward['attribute']['value'];
                break;
                
            case 'beans:cart_discount':
    ...
    public function process_beans_transaction($order_id){
        ...
        foreach($coupon_codes as $code){
            ...
            if(strpos($code, self::uid) !== false) 
                try{
                    ...
                    BeansAPI::call('reward/prepare_use/', $arg);
                    BeansAPI::call('reward/execute/', $arg);
        ...
        try{
            ...
            $arg ['number']     = $total;
            $arg ['rule']       = 'beans:currency_spent:'.get_woocommerce_currency();
            $arg ['unique_id']  = self::uid.$order->id.'_'.$order->order_key;
            
            if($total>0)            
                BeansAPI::call('beans/prepare_add/',$arg);
    ...
    public function confirm_beans_transaction($order_id, $order_status, $new_status){
        ...
        if ( $new_status=='processing' || $new_status=='completed')
        {
            ...
            BeansAPI::call('beans/execute/',$arg);
        }
    ...
    public function render_plugin($page){
        ...
        wp_enqueue_script('beans-plugin-script', $website."/assets/static/js/beans.client.js?public=$beans_public&css=1");
