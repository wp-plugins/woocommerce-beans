...
class WC_Beans{
    ...
    public function get_beans_coupon($coupon, $coupon_code){
        ...
        $max_coupon = $cart_beans_limit_percentage * $cart->subtotal;
        ...
        $coupon_data['apply_before_tax']          = 'yes';
    ...
    public function render_shop_page($page){
        ...
        <span  class="price beans-price">
            ...
            <?php
                printf(
                    __('<span class="beans-unit"> %1$s</span>', 'woocommerce-beans'),
                    smart_beans_format($beans_to_buy,BEANS_DECIMALS).' '.
                    $_SESSION['beans_name']
                );
            ?>
    ...
    public function render_cart_checkout_page($page){
        ...
        if ($this->beans_account__id && $_SESSION['beans_coupon_data'] && $_SESSION['beans_to_redeem'] > 0) {
            $info_html = sprintf( 
                ...
                smart_beans_format($_SESSION['beans_to_redeem'],BEANS_DECIMALS),
                $_SESSION['beans_name']
            );
            ...
            $info_html = sprintf( 
                ...
                '<span class="beans-unit">' .smart_beans_format($account['beans'],BEANS_DECIMALS).' '.
                $_SESSION['beans_name']
            );
            $info_html .= '</span>';
                    ...
                    <td>
                        <div style="padding: 10px; text-align: left;" >
                            <?php
                                 ...
                                 echo "<br/>";
                                 printf(
                                 __('Earn <span class="beans-unit"> %1$s</span> with this purchase.', 'woocommerce-beans'),
                                 smart_beans_format($beans_to_earn,BEANS_DECIMALS).' '.
                                 $_SESSION['beans_name']
                                 );
                            ...
                            <a class="beans-info" href="//www.beans.cards/$<?php echo $this->opt['card_name']; ?>/"  target="_blank"  onclick="Beans.show(); return false;">
                                <?php
                                    ...
                                    printf(
                                        ...
                                        __('<span class="beans-unit"> %1$s </span> // %2$s', 'woocommerce-beans'),

                                        smart_beans_format($_SESSION['beans_rate'],BEANS_DECIMALS).' '.
                                        $_SESSION['beans_name'],
                                        wc_price(1)
                                    );
    ...
    public function render_product_page($page){
        ...
        if (!empty($min_price) && !empty($max_price)){
            ...
            $get_product_msg = sprintf(__('Get this product for  %1$s - %2$s %3$s', 'woocommerce-beans'),'<span class="beans-unit">'.smart_beans_format($beans_to_buy_min,BEANS_DECIMALS), smart_beans_format($beans_to_buy_max,BEANS_DECIMALS),$_SESSION['beans_name']);
            $buy_product_msg = sprintf(__('Buy this product and earn %1$s - %2$s %3$s', 'woocommerce-beans'),'<span class="beans-unit">'.smart_beans_format($beans_to_earn_min,BEANS_DECIMALS),smart_beans_format($beans_to_earn_max,BEANS_DECIMALS),$_SESSION['beans_name']);
            
        }else{
            $beans_to_earn  = (int) ($regular_price * $_SESSION['beans_rule_currency_spent']['beans']);
            $beans_to_buy   = (int) ($regular_price * $_SESSION['beans_rate']);
            
            $get_product_msg = sprintf(__('Get this product for  %1$s', 'woocommerce-beans'),'<span class="beans-unit">'. smart_beans_format($beans_to_buy,BEANS_DECIMALS).' '.$_SESSION['beans_name']);
            $buy_product_msg = sprintf(__('Buy this product and earn %1$s', 'woocommerce-beans'),'<span class="beans-unit">'.smart_beans_format($beans_to_earn,BEANS_DECIMALS).' '.$_SESSION['beans_name']);
