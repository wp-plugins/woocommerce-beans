...
class WC_Beans{
    ...
    function __construct(){
        ...
        $this->filters = array(
            ...
            array('woocommerce_before_my_account',                  'render_account_page',          15, 1),
    ...
    public function initialize(){
        ...
        $this->debug = isset($_GET['beans-debug']) || isset($_SESSION['beans_debug']);
        ...
        if($this->opt['test_mode'] && !current_user_can('manage_woocommerce') && !$this->debug)
        ...
        if((empty($_SESSION['beans_name']) || empty($_SESSION['beans_rate']) || empty($_SESSION['beans_style']))
                && $this->opt['card_id']) {
            $card  = Beans::get('card/' . $this->opt['card_id']);
            $_SESSION['beans_name'] = $card['beans_name'] ;
            $_SESSION['beans_rate'] = $card['beans_rate'] ;
            $_SESSION['beans_style'] = $card['style'] ;
        }
        ...
        if(!empty($_SESSION['beans_style']))
            define('BEANS_SECONDARY_COLOR', $_SESSION['beans_style']['secondary_color']);
    ...
    public function enqueue_scripts(){
        ...
        $data = array(
            ...
            'connect_num' =>  $this->opt['auto_enroll'] ? 2:1,
            'login_url'   =>  $login_url,
...
//        }
    ...
    public function redeem_beans(){
        ...
        if ($account['beans'] < $min_coupon ){
            wc_add_notice(
                sprintf(__( 'Not enough %1$s to redeem.', 'woocommerce-beans' ), $_SESSION['beans_name']).' '. // optimizing for translation
    ...
    public function render_product_page($page){
        ...
        echo '<div class="beans-div-product-page" >';
    ...
    public function render_cart_checkout_page($page){
        ...
        if($_SESSION['beans_account'] && $page == 'account'){
            ...
            $form_html = "<a class='$button_style'  target='_blank' onclick='Beans.show(0, $connect_num); return false;' ".
